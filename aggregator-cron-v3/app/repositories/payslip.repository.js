const { Employee, Payslip1, sequelize } = require("../../models");
...
async function acquireDataSAP(nppOrPn, dateNow, type) {
  console.log(`${process.env.SAP_URL}` +
    `&mode=06&pernr=` +
    nppOrPn +
    `&payment_date=` +
    dateNow +
...
async function acquiredAllData(filter = {}, query = null) {
  ...
  let mapDate = moment(query.periode).format("YYYY-MM-") + requiredDay;
  console.log(moment(query.periode).format("YYYY-MM-DD"));
  let filterAll = {
    attributes: {
      include: [
        "id",

...
const RunFamily = () => {
  ...
  const onFinish = (values) => {
    openModal(`Anda Yakin Ingin Menjalankan Cronjob FAMILY ${values.personnel_number ?? ''} Pada Tanggal ${values.changedate.format('YYYY-MM-DD')} ? Harap Teliti Kembali di Halaman Cek Kiriman Cronjob`, 'warning!', true)
  ...
  const processData = async () => {
    try {
      ...
      const response = await axios.get(`${__VITE_REACT_APP_API_URL__}/cron/run-family${finalRoute}`, config);
        ...
        <div className={classes.content}>
          ...
          <div className={classes['table-container']}>
            ...
            <NewTable columns={[
                ...
                {
                  title: 'Kode Hubungan',
                  dataIndex: 'kode_hubungan',
                  key: 'kode_hubungan',
                  width: '20%'
                },
                {
                  title: 'Nama Keluarga',
                  dataIndex: 'nama_keluarga',
                  key: 'nama_keluarga',
                  width: '20%'
                },
                {

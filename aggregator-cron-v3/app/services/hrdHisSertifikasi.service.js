const repository = require('../repositories/hrdHisSertifikasi.repository');
...
const getData = async (req) => {
    ...
    try {
      ...
      const sertifikasiDataResponse = await repository.getSertifikasiData(sourceClient, req.query.changedate ? req.query.changedate : null);
      
      // Transfer data process if sertifikasiDataResponse not null
      if(sertifikasiDataResponse){
          // Clear mappingDataArray before starting new iteration
          mappingDataArray = [];
          for (const row of sertifikasiDataResponse){

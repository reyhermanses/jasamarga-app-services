const service = require('../../services/master/masterPersonalSubArea.service')
...
async function create(req, res) {
  try {
    ...
    const { description, master_personal_area_id } = req.body

    let insertedData = await service.createData({ description, master_personal_area_id, "created_by": req.api_key.name })

    if (insertedData == "personal area tidak ditemukan") {
...
async function update(req, res) {
  ...
  try {
    ...
    if (!data) {
      ...
      if (updatedData == "personal area tidak ditemukan") {
        return res.status(422).send(sendResponse.unprocessableEntityError(updatedData))
      }
